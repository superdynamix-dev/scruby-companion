...
public final class ScrubyAdminListCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminListCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-list", "Admin: List all companion slots for a player.");
    ...
    protected void execute(
        ...
        if (args.length < 1 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-list <player>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.listSlots(target)));
    }
}
