...
public final class ScrubyAdminSetLevelCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminSetLevelCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-setlevel", "Admin: Set companion level for a player.");
    ...
    protected void execute(
        ...
        if (args.length < 3 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-setlevel <player> <slot> <level>"));
            return;
        }
        ...
        try {
            ...
            ctx.sendMessage(Message.raw("[Scruby-Admin] Invalid level: " + args[2]));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.setLevel(target, slot, level)));
    }
}
