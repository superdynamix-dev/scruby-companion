...
public final class ScrubyAdminSetPathCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminSetPathCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-set-path", "Admin: Set companion path for a player.");
    ...
    protected void execute(
        ...
        if (args.length < 3 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-set-path <player> <slot> <path>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.setPath(target, slot, path)));
    }
}
