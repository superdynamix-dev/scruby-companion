...
public final class ScrubyAdminDeleteCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminDeleteCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-delete", "Admin: Delete a player's companion from a specific slot.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-delete <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.deleteCompanion(target, slot)));
    }
}
