...
public final class ScrubyAdminDespawnCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminDespawnCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-despawn", "Admin: Force despawn a phantom companion.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-despawn <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.forceDespawn(target, slot)));
    }
}
