...
public final class ScrubyAdminPrestigeResetCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminPrestigeResetCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-prestige-reset", "Admin: Reset prestige attempt for a player's companion.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-prestige-reset <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.prestigeReset(target, slot)));
    }
}
