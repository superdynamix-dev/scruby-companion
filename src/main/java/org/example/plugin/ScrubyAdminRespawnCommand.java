...
public final class ScrubyAdminRespawnCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminRespawnCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-respawn", "Admin: Force respawn a companion.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-respawn <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.forceRespawn(target, slot)));
    }
}
