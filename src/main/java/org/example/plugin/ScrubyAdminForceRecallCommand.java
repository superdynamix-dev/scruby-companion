...
public final class ScrubyAdminForceRecallCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminForceRecallCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-force-recall", "Admin: Force recall stationed companion.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-force-recall <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.forceRecall(target, slot)));
    }
}
