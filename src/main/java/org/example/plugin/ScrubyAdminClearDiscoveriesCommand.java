...
public final class ScrubyAdminClearDiscoveriesCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminClearDiscoveriesCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-clear-discoveries", "Admin: Clear all mob discoveries for a player.");
    ...
    protected void execute(
        ...
        if (afterPrefix.isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-clear-discoveries <player>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.clearDiscoveries(target)));
    }
}
