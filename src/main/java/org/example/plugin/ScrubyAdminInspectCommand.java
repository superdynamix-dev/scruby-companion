...
public final class ScrubyAdminInspectCommand extends AbstractPlayerCommand {

    private final ScrubyAdminService adminService;

    public ScrubyAdminInspectCommand(@Nonnull ScrubyAdminService adminService) {
        super("scruby-admin-inspect", "Admin: Inspect a player's companion data.");
    ...
    protected void execute(
        ...
        if (args.length < 2 || args[0].isEmpty()) {
            ctx.sendMessage(Message.raw("[Scruby-Admin] Usage: /scruby-admin-inspect <player> <slot>"));
            return;
        }
        ...
        ctx.sendMessage(Message.raw(adminService.inspectSlot(target, slot)));
    }
}
